...
module.exports = function qpm(opts) {
  ...
  return function processQuery(params, fields, validate) {
    ...
    for (var paramSpec in params) {
      ...
      if (
        paramValues.some(function (v) {
          return v == undefined;
        })
      ) {
        errors.push(
          "Error converting to " + dataType + ": " + params[paramSpec]
        );
      }
    ...
    if (params.__projection) {
      projection = params.__projection.split(",").reduce(function (acc, field) {
        acc[field] = 1;
        return acc;
      }, {});
    }
    if (params.__include_count && params.__include_count == "true") {
      include_count = true;
    }
